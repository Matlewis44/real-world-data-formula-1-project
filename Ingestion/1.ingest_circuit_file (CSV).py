...
dbutils.widgets.text("donnée_entrante", "")
v_data_source = dbutils.widgets.get("donnée_entrante")
...
# On le cherche 
# les fichiers contenus dans le blob container raw
...
# # .csv('/mnt/mathiasf1datalake/raw/circuits.csv')
...
# MAGIC %md
# MAGIC - Très important car particulièrement avantageux 
# MAGIC - une fois les données transformées, il offre une compression efficace, des temps de lecture plus rapides, 
# MAGIC - une meilleure utilisation des ressources de traitement, une compatibilité avec les outils d'analyse 
# MAGIC - qui améliorent les performances et la facilité d'utilisation 
# MAGIC - dans les environnements de traitement et d'analyse de données.
...
# MAGIC %md
# MAGIC 1. Création de la table-managed circuits_v2 dans la bdd
...
# MAGIC %md
# MAGIC ####Anciens chemins

# COMMAND ----------

# On le cherche 
# 1 partition car on fonctionne en single node
...
display(spark.read.parquet(f"{processed_folder_path}/{v_file_date}/circuits"))
